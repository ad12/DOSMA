...
class CommandLineFlagGUI():
    def __init__(self, cmd_line_metadata: dict, **kwargs):
        ...
        self._choices = cmd_line_metadata['choices'] if 'choices' in cmd_line_metadata.keys() else None
    ...
    def draw(self, frame: tk.Frame, **kwargs):
        ...
        """
        draw_cmd = {str: lambda root: self._draw_str_gui(root),
        ...
        l = tk.Label(hbox, text=self._label_format % self._name)
        l.pack(side='left', anchor='nw', padx=padx)
    ...
    def _draw_number_gui(self, root, dtype='float'):
        vcmd = (root.register(self._validate_number),
                             dtype, '%d', '%i', '%P', '%s', '%S', '%v', '%V', '%W')
        return tk.Entry(root, textvariable=self.tk_var, validate='all', validatecommand=vcmd)

    def _draw_int_gui(self, root):
        vcmd = (root.register(self._validate_number),
                             'float', '%d', '%i', '%P', '%s', '%S', '%v', '%V', '%W')
        return tk.Entry(root, textvariable=self.tk_var, validate='all', validatecommand=vcmd)

    def _validate_number(self, dtype, action, index, value_if_allowed, prior_value, text, validation_type, trigger_type, widget_name):
...
class PreferencesManager(metaclass=Singleton):
    def __init__(self):
        ...
        for preference in preferences_metadata.keys():
            self._preference_elements[preference] = CommandLineFlagGUI(preferences_metadata[preference],
                                                                  label_format='%s:\t')
    ...
    def __display_gui(self):
        ...
        apply_settings_button = ttk.Button(hbox, text='Apply Settings', command=lambda: self._apply_settings())
        self.balloon.bind(apply_settings_button, 'Apply settings for this session')
        save_settings_button = ttk.Button(hbox, text='Save Settings', command=lambda: self._save_settings())
